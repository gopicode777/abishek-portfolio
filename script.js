...
function runPageLoader(){
  ...
  function reveal(){
    ...
    const finishPlain = ()=>{
      if(loaderEl){
        loaderEl.style.transition = 'opacity .5s ease';
        loaderEl.style.opacity = '0';
        loaderEl.style.pointerEvents = 'none';
        setTimeout(()=>{ loaderEl.style.display = 'none'; }, 500);
      }
      if(!pageInitialized){ pageInitialized = true; initPage(); }
    };

    if(typeof gsap === 'undefined'){
      // GSAP CDN didn't load (blocked network / offline) — skip the fancy
      // shutter animation and just reveal the page instead of hanging.
      finishPlain();
      return;
    }

    try{
      gsap.timeline({ onComplete:()=>{ if(!pageInitialized){ pageInitialized = true; initPage(); } } })
        .to('.shutter-blade', { scale: 0, transformOrigin:'75px 75px', duration:.9, ease:'power3.inOut', stagger:.03 })
        .to('#loader', { opacity:0, duration:.6, ease:'power2.inOut', pointerEvents:'none' }, '-=.3')
        .set('#loader', { display:'none' });
    }catch(err){
      console.warn('Loader animation failed, revealing page anyway.', err);
      finishPlain();
    }
  }

  // Normal path: reveal shortly after everything has loaded.
  setTimeout(reveal, 900);
  // Safety net: never leave the visitor staring at a stuck loader —
  // if something above (slow images, a blocked CDN script, etc.) takes
  // too long, force the page open after 6 seconds no matter what.
  setTimeout(reveal, 6000);
}

// Prefer 'load' (waits for images) but don't let it block forever —
// if 'load' hasn't fired within 8s of DOM being ready, start anyway.
...
document.addEventListener('DOMContentLoaded', ()=>{
  setTimeout(startLoaderOnce, 8000);
});
...
if(typeof gsap !== 'undefined'){
  gsap.ticker.add(()=>{ rx += (mx-rx)*0.16; ry += (my-ry)*0.16; if(cRing){ cRing.style.left=rx+'px'; cRing.style.top=ry+'px'; } });
}
